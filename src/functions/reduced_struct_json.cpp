...
llvm::json::Object reducedStructJson(
  ...
  if (Declaration.isCompleteDefinition()) {
    ...
    JsonStruct["size"] =
        static_cast<int64_t>(Layout.getSize().getQuantity());
    JsonStruct["alignment"] =
        static_cast<int64_t>(Layout.getAlignment().getQuantity());

    Fields.reserve(std::distance(Declaration.field_begin(),
                                 Declaration.field_end()));
    ...
    for (const clang::FieldDecl *Field : Declaration.fields()) {
      ...
      JsonField["offset"] = static_cast<int64_t>(
          OffsetBits / Context.getCharWidth());
